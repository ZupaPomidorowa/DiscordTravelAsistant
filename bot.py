...
async def find_route(ctx, origin, dest, num: int):
    ...
    top_attractions, attraction_names = find_attractions(latlong_origin, latlong_dest, num)

    url = create_url(latlong_origin, latlong_dest, top_attractions)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send('Missing arguments. Usage: !find_route <origin> <destination> <number_of_attractions>')
